...
# Make predictions on validation dataset1
...
cm_labels = 'IN IP LY NA OP SP'.split()
print('labels:', cm_labels)
print(LinearDiscriminantAnalysis(labels_validation, predictions))
print()
print('Classification report:')
print(classification_report(labels_validation, predictions))
...
et_clf.fit(feats_train, labels_train)
feature_selection = SelectFromModel(et_clf, prefit=True)
print('Old features:', names)
print('Support:', feature_selection.get_support())
feats_train_new = feature_selection.transform(feats_train)
feats_valid_new = feature_selection.transform(feats_validation)
new_clf = ExtraTreesClassifier()
new_clf.fit(feats_train_new, labels_train)
new_preds = new_clf.predict(feats_valid_new)
print('Accuracy:', accuracy_score(labels_validation, new_preds))
print()
print('Linear Descrimination Analysis:')
cm_labels = 'IN IP LY NA OP SP'.split()
print('labels:', cm_labels)
print(LinearDiscriminantAnalysis(labels_validation, new_preds))
print()
print('Classification report:')
print(classification_report(labels_validation, new_preds))
